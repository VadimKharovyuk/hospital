...
public class RabbitMQListener {
    ...
    public void receiveMessage(Patient patient) {
        // Обработка сообщения
        System.out.println("1 Received  message: " + patient);
    }
    @RabbitListener(queues = "hospital_fanout_queue")
    public void receiveMessage1(Patient patient) {
        // Обработка сообщения
        System.out.println(" 2 Received message: " + patient);
    }


    @RabbitListener(queues = "hospital_fanout_queue")
    public void receiveMessage(Client client) {
        System.out.println("3  слушатель Fanout_ hospital_fanout_queue  " + client);

    }
    @RabbitListener(queues = "hospital_fanout_queue")
    public void receiveMessage1(Client client) {
        System.out.println("4 слушатель Fanout_: _ hospital_queue  " + client);
